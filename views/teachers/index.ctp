<div class="teachers index">
<h2><?php __('Teachers');?></h2>
<p>
<?php
echo $paginator->counter(array(
'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
));
?></p>
<table cellpadding="0" cellspacing="0">
<tr>
	<th><?php echo $paginator->sort('id');?></th>
	<th><?php echo $paginator->sort('name');?></th>
	<th><?php echo $paginator->sort('contact');?></th>
	<th><?php echo $paginator->sort('designation_id');?></th>
	<th><?php echo $paginator->sort('created');?></th>
	<th><?php echo $paginator->sort('modified');?></th>
	<th class="actions"><?php __('Actions');?></th>
</tr>
<?php
$i = 0;
foreach ($teachers as $teacher):
	$class = null;
	if ($i++ % 2 == 0) {
		$class = ' class="altrow"';
	}
?>
	<tr<?php echo $class;?>>
		<td>
			<?php echo $teacher['Teacher']['id']; ?>
		</td>
		<td>
			<?php echo $teacher['Teacher']['name']; ?>
		</td>
		<td>
			<?php echo $teacher['Teacher']['contact']; ?>
		</td>
		<td>
			<?php echo $html->link($teacher['Designation']['title'], array(
			'controller' => 'designations', 
			'action' => 'view', 
			$teacher['Designation']['id'])); 
			?>
		</td>
		<td>
			<?php echo $time->niceShort($teacher['Teacher']['created']); ?>
		</td>
		<td>
			<?php echo $time->niceShort($teacher['Teacher']['modified']); ?>
		</td>
		<td class="actions">
			<?php echo $html->link(__('View', true), array('action' => 'view', $teacher['Teacher']['id'])); ?>
			<?php echo $html->link(__('Edit', true), array('action' => 'edit', $teacher['Teacher']['id'])); ?>
			<?php echo $html->link(__('Delete', true), array('action' => 'delete', $teacher['Teacher']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $teacher['Teacher']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
</table>
</div>
<div class="paging">
	<?php echo $paginator->prev('<< '.__('previous', true), array(), null, array('class'=>'disabled'));?>
 | 	<?php echo $paginator->numbers();?>
	<?php echo $paginator->next(__('next', true).' >>', array(), null, array('class' => 'disabled'));?>
</div>
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('New Teacher', true), array('action' => 'add')); ?></li>
		<li><?php echo $html->link(__('List Teachers', true), array('controller' => 'teachers', 'action' => 'index')); ?> </li>
		<li><?php echo $html->link(__('New Designation', true), array('controller' => 'teachers', 'action' => 'add')); ?> </li>
		<li><?php echo $html->link(__('List Contacts', true), array('controller' => 'contacts', 'action' => 'index')); ?> </li>
		<li><?php echo $html->link(__('New Contact', true), array('controller' => 'contacts', 'action' => 'add')); ?> </li>
	</ul>
</div>
